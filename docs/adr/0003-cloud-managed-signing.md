...
Secrets: `ASC_KEY_ID`, `ASC_ISSUER_ID`, `ASC_PRIVATE_KEY_P8` (the full `.p8` text), `APPLE_TEAM_ID`. The app links only system frameworks and the static `IglooCore` package; embedded frameworks are excluded because they break the unsigned-archive, signed-export flow.
