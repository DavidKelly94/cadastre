...
6. **The contract grows, the version does not.** [ADR-0021](0021-session-format-is-the-only-cross-language-contract.md) makes `docs/session-format.md` the only cross-language contract, so a plan written by the app and read by the pipeline is specified there rather than in a second document. The addition is project-level and touches no session field, so `format_version` stays 3. It documents the file `vividhome plan add` already writes, extended with `source` and `rooms`; the existing field names stay, including calibration being the presence of `metres_per_pixel` and `origin_px` rather than a separate flag that could contradict them.
