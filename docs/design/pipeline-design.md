...
pipeline/
  ...
  igloo/
    ...
    validate.py             rules from session-format.md §10
...
Implements `session-format.md` §10 exactly; prints a table (keyframes, stills, markers by ID with observation counts, landmarks by label, tracking-limited seconds, depth validity %, bytes) and `OK` / `WARN` / `ERROR` lines; exit code 1 on any error. `--json` writes `derived/validate.json`.
...
6. `--check-anchor-frame`: compares ARKit `markers.jsonl` anchors (converted with `R_am` from `session-format.md` §7) against PnP results and prints the per-axis agreement; used once on the first real capture to confirm the anchor-frame constant.
