...
A note on the name, because it shapes the visuals. **VividHome** reads plainly in English and needs no gloss, which is the opposite of the name it replaced ([ADR-0024](../adr/0024-name-vividhome.md)). "Vivid" is a claim about the record, not about decor: the point is that what the walls hide stays sharply visible years later. Keep the visual language away from interior styling and paint chips, which is the wrong reading the word invites. The wordmark is one word, capital V and capital H, never "Vivid Home" and never abbreviated to "VH" outside the marker IDs. "VividHome AI" is permitted as a descriptor in directory listings and title tags, but is not the wordmark.
