...
/// depth buffer all live in CadastreCore or the recorder. What is left is Apple's
...
final class ARSessionController: NSObject, ObservableObject {
  ...
  enum Unsupported: Equatable {
    ...
    var message: String {
      switch self {
      case .noWorldTracking:
        return "This iPhone cannot run world tracking, which Cadastre needs to record."
      case .noSceneDepth:
        return "This iPhone has no LiDAR scanner. Cadastre needs one: an iPhone 15 Pro or newer."
      case .noSceneReconstruction:
        return "This iPhone cannot build a scene mesh. Cadastre needs an iPhone Pro with LiDAR."
      }
    }
  }
