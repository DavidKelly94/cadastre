...
// Writes a session using nothing but CadastreCore, so the pipeline's validator
...
guard arguments.count >= 2 else {
  FileHandle.standardError.write(Data("usage: cadastre-fixture <output-directory>\n".utf8))
  exit(2)
}
...
let manifest = Manifest.starting(
  ...
  device: DeviceInfo(
    model: "fixture", iosVersion: "0", appVersion: CadastreCore.version, appBuild: "0"),
