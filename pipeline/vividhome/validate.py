...
def _is_marker_id(value: str) -> bool:
    """Rule 8: marker ids match ``VH-\\d{3}``."""
    return len(value) == 6 and value.startswith("VH-") and value[3:].isdigit()
    ...
    try:
        for observation in session.markers():
            ...
            if not _is_marker_id(observation.marker_id):
                report.add(ERROR, 8, f"marker id {observation.marker_id!r} does not match VH-NNN")
    except SessionError as error:
        report.add(ERROR, 2, str(error))
